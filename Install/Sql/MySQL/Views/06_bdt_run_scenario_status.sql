CREATE OR REPLACE VIEW bdt_run_scenario_status AS
SELECT
...
FROM
    bdt_run_scenario_stats rs
        ...
        INNER JOIN (
        SELECT
            rsn2.oid AS run_scenario_oid,
            MAX(rsn2.finished_on) AS max_finished_on
        FROM
            bdt_run_scenario rsn2
        GROUP BY
            rsn2.oid
    ) lfo ON lfo.run_scenario_oid = rsn.oid AND lfo.max_finished_on = rsn.finished_on
